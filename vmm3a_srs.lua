...
function srsvmm_proto.dissector(buffer,pinfo,tree)
	pinfo.cols.protocol = string.format("SRSVMM3a %2.2f MHz",1000/clock_period);
	...
	if (fc0 == 0) then
		...
		if dataid == 0x564d33 then
			...
			if protolen >= 16 then
				...
				local marker_id = 0
				local n_markers = 0
				local last_marker1 = 0
				local last_marker2 = 0
				local rate = 0
				local t_start = 0;
				local t_end = 0;
				for i=1,hits do
					...
					-- marker
						...
						if last_marker1 < timestamp1 or (last_marker1 == timestamp1 and last_marker2 < timestamp2) then
						...
						last_marker1 = timestamp1
						last_marker2 = timestamp2
					else
						-- hit
						hit_id = hit_id + 1
						--data 2 (16 bit):
						...
						if hit_id == 1 then
							t_start= (offset*4096+bcid)*clock_period
						end
						t_end = (offset*4096+bcid)*clock_period
						local hit = 0			
						if adc < 16 then
							local latency = 0
							if trg-bcid >= -8 then
				...
				t_end = t_end + n_markers*4096*16*clock_period
			    	rate = 1000000*hit_id/(t_end-t_start)
				freq = 1000/clock_period
				bitrate = rate*48/1000
		  		pinfo.cols.info = string.format("FEC: %d, Hits: %3d, Markers: %3d, Clock: %2.2f MHz, Hit rate: %6.2f kHz, Bit rate: %6.2f Mbit/s", fecid, hit_id, marker_id, freq, rate, bitrate)
