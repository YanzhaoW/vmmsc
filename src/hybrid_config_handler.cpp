...
hybrid_config_handler::hybrid_config_handler(MainWindow *top,QObject *parent) : root1{top}, QObject(parent)
{
    getcwd(ExecPath,sizeof(ExecPath));
}


bool hybrid_config_handler::LoadAllHybridConf(std::string filename){
    return GenericAllHybridConf(1,filename);
}

bool hybrid_config_handler::WriteAllHybridConf(std::string filename){
    return GenericAllHybridConf(0,filename);
}

bool hybrid_config_handler::LoadSingleHybridConf(const char* filename, unsigned short daq, unsigned short fec, unsigned short hdmi, unsigned short hybrid){
    return GenericSingleHybridConf(1, filename, daq, fec, hdmi, hybrid);
}

bool hybrid_config_handler::LoadSingleHybridConf(const char* filename){//exact file name must be given!
...
bool hybrid_config_handler::WriteSingleHybridConf(const char* filename, unsigned short daq, unsigned short fec, unsigned short hdmi, unsigned short hybrid){
    return GenericSingleHybridConf(0, filename, daq, fec, hdmi, hybrid);
}

bool hybrid_config_handler::WriteSingleHybridConf(const char* filename){//exact file name must be given!
...
bool hybrid_config_handler::GenericAllHybridConf(bool load, std::string filename){
...
bool hybrid_config_handler::GenericSingleHybridConf(bool load, const char* filename, unsigned short daq, unsigned short fec, unsigned short hdmi, unsigned short hybrid){
...
bool hybrid_config_handler::LoadHybridConfig(std::string fname){ //load the VMM configuration from file
    std::string empty = "";
    ...
    while (!f.eof() ){
        ...
        if (s == empty && val == empty) break; // for empty line at end of file
            ...
            if (!root1->daq[daq].fec[fec].hdmi[hdmi].hybrid[hybrid].SetReg(a,b)) return false;
        }
...
bool hybrid_config_handler::WriteHybridConfig(std::string fname, unsigned short daq, unsigned short fec, unsigned short hdmi, unsigned short hybrid){
    ...
    f << "\n";
    for(auto const entr: root1->daq[daq].fec[fec].hdmi[hdmi].hybrid[hybrid].m_hybrid){
...
hybrid_config_handler::~hybrid_config_handler(){
